...
8. **What's next** — after confirming the result, show the shared "What's next" footer from `commands/_whats-next.md` (showing all three, with `/modes-map` marked "(run again)") so the user discovers the other tools.
